...
/**
 * Emit Python for a workspace. Only stacks connected under a `on_program_start`
 * hat run inside main(); procedure definitions become top-level `def`s via
...
export function workspaceToPython(workspace: Workspace): string {
  ...
  // Emit main() body from every hat's downstream chain.
  const chunks: string[] = [];
  for (const top of workspace.getTopBlocks(true)) {
