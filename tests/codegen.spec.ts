...
describe("workspaceToPython", () => {
  it("emits header for empty workspace", () => {
    const ws = new Blockly.Workspace();
    const py = workspaceToPython(ws);
    expect(py).toContain("# Generated by LEGO-Hub-blocks");
  ...
  it("hat block with no downstream emits header only", () => {
    const ws = makeWorkspace({
      blocks: {
        languageVersion: 0,
    ...
    expect(py.trim()).toBe("# Generated by LEGO-Hub-blocks");
  });
});
